...
Those require the actual GenLayer Studio or testnet (see
`tests/test_gltest_integration.py` and the README for how to run
real end-to-end tests there with the `gltest` framework).
"""
...
class _EqPrinciple:
    ...
    GenLayer Studio/testnet - see tests/test_gltest_integration.py.
    """
...
class _Contract:
    ...
    them by hand in `__init__` (the original, accepted contract this
    is based on relies on exactly this behavior for its TreeMap
    fields). This stub reproduces that by scanning class annotations
    at construction time and pre-populating any TreeMap/DynArray
    fields with empty instances before the contract's own `__init__`
    runs.
    """
