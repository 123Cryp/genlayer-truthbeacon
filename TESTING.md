...
TruthBeacon v2 has three tiers of testing: offline unit tests (87, all passing, run in this repository), an unexecuted live-integration example (`gltest_integration_example.py`), and real deployment evidence on GenLayer Studio. This document explains what each tier proves and what it does not.
...
**Result: 87/87 passing.** These run against a small local stub of the `genlayer` SDK (`tests/genlayer_stub/`) — no GenLayer node, network access, or real LLM required. `gl.nondet.web.render` and `gl.nondet.exec_prompt` are monkeypatched per test to simulate specific scenarios deterministically.

### 2a. Unit tests (pure functions, no mocking needed)

| File | Tests | Function under test | What it proves |
|---|---|---|---|
| `test_domain_extraction.py` | 18 | `_extract_domain`, `_registrable_domain` | The four required subdomain-independence cases; multi-part suffix handling; IPv6/trailing-dot edge cases |
...
| `test_aggregation.py` | 9 | `_aggregate` | Every branch of the final-verdict decision rule, including majority-with-dissent |
...
| `test_prompt_and_consensus.py` | 11 | `_build_prompt`, `EQUIVALENCE_PRINCIPLE` | Every guardrail phrase is actually present in the model prompt; the equivalence principle references real schema fields |

These 57 tests require zero mocking — they call deterministic functions directly with crafted inputs and assert exact outputs.
...
| `test_end_to_end.py` | 13 | The full `submit_claim` → `get_claim` pipeline, with `gl.nondet.*` mocked: successful verification, refutation, disputes, duplicates, failures, adversarial sources |
...
**Total: 87 tests across 8 files.** See `tests/README.md` for the full coverage checklist mapping every requested scenario (valid claims, insufficient sources, duplicate domains, malformed URLs, timeouts, empty/malformed content, low-credibility domains, boundary values, storage persistence) to its test file.
...
It sketches the same core scenarios (successful verification, conflicting evidence, duplicate domains, failed fetches, malicious sources, and an all-denylisted rejection) using GenLayer's own `gltest` mocking framework (`MockedWebResponse` / `MockedLLMResponse`), based on published `gltest` documentation. The exact API surface used (`get_contract_factory`, `factory.deploy`, the `mock_web_responses=`/`mock_llm_responses=` signature) has **not** been confirmed against an installed `gltest` version. Treat it as a starting point to adapt and validate, not a verified passing suite.
...
- Real-world fetch failures occur and are handled gracefully exactly as the offline tests predict (britannica.com returned `inaccessible` in two separate live transactions).
...
|---|---|
| Valid claims | Tier 1 (`test_end_to_end.py`), Tier 3 (tx #2) |
| Insufficient sources | Tier 1 (`test_input_validation.py`), Tier 3 (tx #5) |
| Duplicate domains | Tier 1 (`test_domain_extraction.py`, `test_end_to_end.py`), Tier 3 (tx #4) |
| Malformed URLs | Tier 1 (`test_domain_extraction.py`) |
| Inaccessible URLs | Tier 1 (`test_end_to_end.py`), Tier 3 (tx #3, #6) |
